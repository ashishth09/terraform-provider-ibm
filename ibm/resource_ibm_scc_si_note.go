...
func resourceIBMNoteCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if len(relatedURLInterface) != 0 {
		relatedURLObjectList := make([]findingsv1.APINoteRelatedURL, 0)
		for _, relatedURL := range relatedURLInterface {
			relatedURLObject := findingsv1.APINoteRelatedURL{}
	...
	if len(reportedByInterface) != 0 {
		reportedByObject := findingsv1.Reporter{}
	...
	if len(cardInterface) != 0 {
		...
		cardElementsList := make([]findingsv1.CardElementIntf, 0)
		for _, cardElement := range cardInterface[0].(map[string]interface{})["elements"].([]interface{}) {
			cardElementObject := findingsv1.CardElement{}
			...
			cardValueTypeObject := findingsv1.NumericCardElementValueType{}
			...
			cardValueTypesObjectList := make([]findingsv1.ValueTypeIntf, 0)
			for _, cardValueType := range cardElement.(map[string]interface{})["value_types"].([]interface{}) {
				cardValueTypeObject := findingsv1.ValueType{}
				...
				cardValueTypeObject.FindingNoteNames = cardValueTypeFindingNoteNames
				cardValueTypesObjectList = append(cardValueTypesObjectList, findingsv1.ValueTypeIntf(&cardValueTypeObject))
			}
			cardElementsList = append(cardElementsList, findingsv1.CardElementIntf(&cardElementObject))
			cardElementObject.ValueTypes = []findingsv1.ValueTypeIntf(cardValueTypesObjectList)
		}
		cardObject.Elements = []findingsv1.CardElementIntf(cardElementsList)
		createNoteOptions.Card = &cardObject
	}

	if len(sectionInterface) != 0 {
		sectionObject := findingsv1.Section{}
	...
	log.Println(fmt.Sprintf("found accountId specified, using %s", accountID))
	if accountID == "" {
		accountID = userDetails.userAccount
		log.Println(fmt.Sprintf("accountId isn't specified, using %s", accountID))
	}
...
func resourceIBMNoteRead(d *schema.ResourceData, meta interface{}) error {
	...
	if note.Finding != nil {
		noteFindingTypeObjectList := make([]map[string]interface{}, 0)
	...
	if note.Card != nil {
		...
		for _, noteCardElement := range note.Card.Elements {
			...
			noteCardElementObject["kind"] = noteCardElement.(*findingsv1.CardElement).Kind
			noteCardElementObject["text"] = noteCardElement.(*findingsv1.CardElement).Text
			noteCardElementObject["default_time_range"] = noteCardElement.(*findingsv1.CardElement).DefaultTimeRange
			noteCardElementObject["default_interval"] = noteCardElement.(*findingsv1.CardElement).DefaultInterval
			noteCardElementsList = append(noteCardElementsList, noteCardElementObject)
			if noteCardElement.(*findingsv1.CardElement).ValueType != nil {
				noteCardElementValueTypeObjectList := make([]map[string]interface{}, 1)
				noteCardElementValueTypeObject := map[string]interface{}{}
				noteCardElementValueTypeObject["kind"] = noteCardElement.(*findingsv1.CardElement).ValueType.Kind
				noteCardElementValueTypeObject["finding_note_names"] = noteCardElement.(*findingsv1.CardElement).ValueType.FindingNoteNames
				noteCardElementValueTypeObject["kpi_note_name"] = noteCardElement.(*findingsv1.CardElement).ValueType.KpiNoteName
				noteCardElementValueTypeObject["text"] = noteCardElement.(*findingsv1.CardElement).ValueType.Text
			...
			if noteCardElement.(*findingsv1.CardElement).ValueTypes != nil {
				noteCardElementValueTypesList := make([]map[string]interface{}, 0)
				for _, noteCardElementValueTypes := range noteCardElement.(*findingsv1.CardElement).ValueTypes {
					noteCardElementValueTypesObject := map[string]interface{}{}
					noteCardElementValueTypesObject["kind"] = noteCardElementValueTypes.(*findingsv1.ValueType).Kind
					noteCardElementValueTypesObject["finding_note_names"] = noteCardElementValueTypes.(*findingsv1.ValueType).FindingNoteNames
					noteCardElementValueTypesObject["kpi_note_name"] = noteCardElementValueTypes.(*findingsv1.ValueType).KpiNoteName
					noteCardElementValueTypesObject["text"] = noteCardElementValueTypes.(*findingsv1.ValueType).Text
				}
